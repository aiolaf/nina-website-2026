...
export function OfferUploadPanel({
  ...
  async function loadNormalized() {
    ...
    setResult(null)
    try {
      const res = await api.offersLoad(klant, demo.id)
      setResult(res)
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
    ...
    <div className="rounded-2xl border border-[var(--color-line)] bg-white">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-line)] px-5 py-3">
        <div className="flex items-center gap-2">
          <span className="font-medium">Prijsvergelijk vullen</span>
          <span className="text-xs text-[var(--color-ink-soft)]">
            laad de genormaliseerde offertes uit stap 1 → in het template
          </span>
        </div>
      ...
      <div className="px-5 py-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="text-sm font-medium">
                  Genormaliseerde offertes uit stap 1
                </div>
                <div className="text-xs text-[var(--color-ink-soft)]">
                  In stap 1 zijn de offertes al samen uitgelezen en genormaliseerd.
                  Laad ze hier direct in het prijsvergelijk — geen upload nodig.
                </div>
              </div>
              <button
                onClick={loadNormalized}
                disabled={busy}
              ...
              >
                {busy ? 'Bezig…' : '↧ Genormaliseerde offertes laden'}
              </button>
            </div>
          </div>
        )}
        ...
        <details className="mb-2">
          <summary className="cursor-pointer text-xs text-[var(--color-ink-soft)] transition hover:text-[var(--color-ink)]">
            Of: offertes zelf ophalen / uploaden (SharePoint · Outlook · upload)
          </summary>
          <div className="mt-3">
        {mode === 'sharepoint' && (
          <div>
        ...
        {busy && (
          <div className="mt-4">
            <Spinner label="Bezig met inladen en het prijsvergelijk vullen…" />
          </div>
        )}
        {error && (
          <div className="mt-4">
